...
probe_module_t module_icmp6_echo_full;
...
static int icmp6_echo_full_global_init(struct state_conf *conf) {
    ...
    module_icmp6_echo_full.packet_length = sizeof(struct ether_header) +
                                         sizeof(struct ip6_hdr) + ICMP6_MINLEN +
                                         icmp6_payload_len;
    assert(module_icmp6_echo_full.packet_length <=
           1500 + sizeof(struct ether_header));
    module_icmp6_echo_full.pcap_snaplen =
...
static int icmp6_echo_full_global_cleanup(UNUSED struct state_conf *xconf,
...
static int icmp6_echo_full_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                                     UNUSED void **arg_ptr) {
...
static int icmp6_echo_full_make_packet(void *buf, UNUSED size_t *buf_len,
...
static void icmp6_echo_full_print_packet(FILE *fp, void *packet) {
...
static int icmp6_echo_full_validate_packet(const struct ip *ip_hdr, uint32_t len,
                                         int *is_repeat) {
...
static void icmp6_echo_full_process_packet(const u_char *packet, uint32_t len,
                                         fieldset_t *           fs,
...
static fielddef_t fields[] = {
    {.name = "success",
     .type = "int",
     .desc = "did probe module classify response as a reply "},
    {.name = "clas",
     .type = "string",
...
probe_module_t module_icmp6_echo_full = {
    .ipv46_flag    = 6,
    .name          = "icmp6_echo_full",
    .packet_length = 14 + 40 + 8 + 8,
    .pcap_filter   = "icmp6 && (ip6[40] == 129 || ip6[40] == 3 || ip6[40] == 1 "
                   "|| ip6[40] == 2 || ip6[40] == 4)",
    .pcap_snaplen    = 14 + 2 * (40 + 8) + 8,
    .port_args       = 0,
    .global_init     = &icmp6_echo_full_global_init,
    .close           = &icmp6_echo_full_global_cleanup,
    .thread_init     = &icmp6_echo_full_thread_init,
    .make_packet     = &icmp6_echo_full_make_packet,
    .print_packet    = &icmp6_echo_full_print_packet,
    .process_packet  = &icmp6_echo_full_process_packet,
    .validate_packet = &icmp6_echo_full_validate_packet,
